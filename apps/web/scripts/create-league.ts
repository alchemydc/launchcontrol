...
//     [--logo-url <http(s) url>]
//
// --gate defaults to "optional". --gate required is allowed: per-league membership
// gating (decideLeagueAccess) resolves access at request time (see src/lib/session.ts),
// so any created league may carry a "required" gate.
//
