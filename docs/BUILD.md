...
`DriverHistoryRow` gained `seasonYear`, and `eventDetailInclude`/`combinedSessionInclude` gained `season.year`/`season.name`: classing is per (league, season), the driver page's rows can span both, and a season may straddle a calendar year, so it is not derivable from `Event.date`.
