...
**RMsolo driver identity (cross-source, PR #146/#149).** The name-only path above was scoped as a rare legacy-file fallback; for the RMsolo pipeline it is the *normal* path, and the accepted risk is correspondingly wider. An RMsolo results PDF never prints a member number, so **every** entry it parses is blank-member and hashes as `SHA256(|first|last)`. Once the `.axdb`/MSR path attaches a `memberNum` to the same human's `Driver` row — drivers are not league-scoped, so the two pipelines share rows — that row's `identityHash` becomes member-inclusive and the blank-member lookup can never match it again. Before the fix, `ingestRmsoloEvent` therefore minted a second `Driver` on every RMsolo ingest and split the person's history by source. It now falls back to `nameOnlyHash` on an `identityHash` miss and merges into the single populated `Driver` sharing the full name, leaving that better-identified row untouched.

Two consequences worth stating plainly rather than inheriting from the MVP note above:

- **The collision risk is no longer blank-to-blank only.** A name-only match now resolves *across sources*, so an RMsolo entrant merges into a populated member row that a different real person could own. The "exactly one candidate" guard only rules out ambiguity the DB can already see: 0 or ≥2 populated candidates leaves a separate blank-member row rather than guessing, but a single wrong same-name match is not detectable and is still accepted at club scale.
- **The fix stops new forks; it does not heal old ones.** A blank-member `Driver` already in the DB still wins the `identityHash` lookup and never reaches the fallback, and rows predating `nameOnlyHash` carry `null` there and cannot be matched at all (the full surname is never stored, so they can't be backfilled). Repairing pairs already forked in prod is a separate backfill.

`ingestRmsoloEvent` needs no in-file ambiguity guard, unlike `ingestAxdb`: with `memberNum` pinned to `null`, `identityHash` and `nameOnlyHash` are both injective in the same normalized `(first, last)` pair, so two same-named entrants in one PDF collapse into one identity upstream and cannot reach the fallback separately. Covered by `apps/web/tests/rmsolo-driver-identity.test.ts`.
...
- `scripts/classing-import.ts` (`pnpm --filter web classing:import --league <slug> <classing.yml>`) — converts upstream YAML, repairing its known defects loudly rather than silently: a trim object keyed `trims:` instead of `trim:`, a placeholder vehicle with nothing but a `type:`, and `version: 0.1` (a YAML float) meaning `.1`. A vehicle with **no `years:` block at all** is not a defect — it means "every year" (upstream's `non-Porsche` → `TO` row: a class defined by what a car *isn't* has no model years to bound), so it imports as `years: null`, renders bare the way the published table renders it, matches every year in lookup, and makes the picker answer its own year step with "Any year". Writes no DB rows, so no audit entry. `yaml` is a devDependency — the converter is build-time only and never ships to the runtime bundle.
...
Verified against the published table: the 2026 grouping is line-for-line identical apart from that displacement qualifier, including the cases the collapse logic exists for — `996.1`/`996.2` merging into one `996 911 · 1999-2004` line because their trims match, `987.1` (S) and `987.2` (S, R) staying separate because theirs don't, and `992 911 · 2020+` keeping its open end when merged.

**Resyncing.** `classing:import` is re-run against a new upstream revision and the JSON diff committed; the check is that the regenerated table still matches the published one. The 2026-08-26 resync (55 vehicles, 146 assignments, up from 47/121) pulled in Boxster and Cayman split into separate models per generation, `982` retyped `718 (982)`, GT4/Spyder split into base and RS entries with their own year ranges, the `968` placeholder filled in, and the `non-Porsche` → `TO` row. It also carried back a fix we sent upstream ([rmr-pca-classing#1](https://github.com/enginerdify/rmr-pca-classing/pull/1)): the Macan had inherited the Cayenne's `from: 2003`, claiming eligibility for eleven model years of a car that first sold as a 2015 model.
...
2. `Driver.nameOnlyHash` — matched only when **exactly one** Driver carries the hash **and that row is unclaimed**, mirroring ingest's own merge/adopt rule: 0 or ≥2 candidates means we don't guess. Two humans sharing a full name therefore resolve to nothing rather than to each other's results, and the `@unique` on `msrUid` means one MSR user can only ever own one row. Both the read and the write count over *every* row carrying the hash rather than the unclaimed subset (PR #176 review): scoping the count to unclaimed rows makes ambiguity depend on ownership, so a claimed row plus an unclaimed row sharing one name would look like a single confident candidate — and a sole row already claimed by someone else would resolve as *this* viewer's, showing them another person's stats.
...
**The card is scoped to the leagues this session may open.** `/me` has no league gate of its own, so it resolves each of the driver's leagues through `checkLeagueAccess` and both summarizes and links only those — otherwise a driver who is not an admitted member of a `required` league would see an event count and best finish for it while the stats route redirected them.

**Where the link points** depends on the accessible leagues, because `/drivers/[id]` gates on the *deployment default* league: default league not accessible → `/l/<their-league>/drivers/<id>`, with the summary pinned to that same single league so the card never counts events the destination won't show (there is no multi-league route that doesn't gate on the default); more than one accessible league → `/drivers/<id>?league=all`; otherwise the plain route. Cross-league aggregation needed no new code — `buildDriverHistory` already accepts `leagueIds: "all"` and the driver page's filter already understands the param.
...
| 7 | Drivers without `member_num` produce duplicate rows under redaction. Acceptable for MVP? | Identity-hash strategy dedupes drivers without `member_num` by name; residual cross-event name-collision risk accepted for MVP. **Widened 2026-08-21 (#146/#149):** RMsolo PDFs are blank-member for every entry, so name-only resolution is a whole pipeline's normal path and now merges *across* sources into populated member rows — same accepted risk, materially larger surface. See "RMsolo driver identity". | 2026-05-23 | M1.10 |
