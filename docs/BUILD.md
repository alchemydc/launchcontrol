...
- **2024 AxWare→VisualAX transition artifacts.** Two 2024 exports have `member_num` blank for every driver, splitting each human into a separate `Driver` row per event; `apps/web/scripts/repair-axdb-identity.ts` is the operator repair path — it cross-references other exports by (first name, last name) to backfill a blank `member_num` when exactly one candidate exists, writing patched copies rather than touching originals. Separately, `member_num` sometimes drifts across exports for the same person by a `verified` suffix (`N` / `N verified` / `N-verified`); ingest strips this via `normalizeMemberNum()` (see Driver identity below) so it never causes a split identity. Some pre-2025 exports also lack the `events.timing_mode` / `typical_time` columns entirely — harmless, since ingest never reads them.
