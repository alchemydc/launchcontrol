...
- **Data invariants (RMR / AxWare convention):**
  ...
  - **Combined (same-date, multi-session) events (M1.15).** Events sharing a calendar date are auto-grouped into one scoring event — no schema change, no ingest flag, no admin linking step. A combined event counts **once** toward the season's `N` (and therefore the `floor(N/2)+1` qualifying threshold). A driver earns points only when they have a countable (CLEAN, per `bestCorrectedMsForEntry`) time in **every** session of the group, **in the same class**; class mismatch across sessions or a missing session forfeits that scoring group entirely (fail-safe — shouldn't occur under RMR convention). The redaction/PII rule applies identically to the combined-standings page. As of M1.16, `N` is `max(planned scoring dates for the year, actual scoring groups ingested)`, where the planned figure comes from the per-year `PLANNED_SEASON_EVENTS` map, so a mid-season count can't shrink the threshold below what the full planned season will require.
...
- Series scoring rules as data: RMR's qualifying-threshold formula (`floor(N/2) + 1` of N season events) lives in code as of M1.13; multi-car / multi-class participation is allowed as of M1.14 (the M1.13 single-car-per-season constraint was reversed by the AX chair). Since M1.16, N is raised to a planned season size via the `PLANNED_SEASON_EVENTS` code map when it exceeds the actual count ingested so far — an interim, code-level stand-in for the planned-events-per-season figure a future `RuleSet` would hold as data. Future regions or rule changes would want a per-season `RuleSet` (events-counted formula, points formula, drop-week count, tiebreakers, single-car vs multi-car rule, planned event count).
