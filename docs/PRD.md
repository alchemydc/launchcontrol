...
**Stack:** Next.js (App Router) · TypeScript · React · Tailwind · Prisma + SQLite · Vercel
...
- **AxWare** — desktop timing software used at-event by RMR. Emits a SQLite database file with the extension `.axdb` after each event (confirmed via the `.axlog.txt` files in `2026_season_data/`).
...
- **Local ingest CLI (M1)** — `pnpm ingest <path-to-axdb>` reads the source SQLite read-only, normalizes into the app DB. The two committed `2026_season_data/*/.axdb` files act as fixtures.
...
Observed by running `.schema` and sample `SELECT`s against both `2026_season_data/*/*.axdb` files. Both files share the same schema.
...
Each committed `.axdb` so far contains a **single event** (id=1) with ~76 drivers and ~622 runs.
...
### M0 — Scaffold (target: 1 session, no blockers)

- `pnpm create next-app` with TypeScript, App Router, Tailwind, ESLint.
- Add Prisma + SQLite; check `prisma/schema.prisma` and an initial migration into the repo.
- Link the repo to Vercel; verify preview deploys on PR.
- `tsconfig.json`: `"strict": true`, `"noUncheckedIndexedAccess": true`.
- `2026_season_data/` already committed; treat as fixtures (not gitignored).

### M1 — Static leaderboard (target: 1 session, no blockers)

- Implement `scripts/ingest.ts` (CLI) per §2.5.
- Ingest both committed `.axdb` files into the dev DB.
- Build `/events/[slug]` rendering a sortable leaderboard (raw, PAX, class) with class filter.
- **This is the smoke test for the entire AxWare pipeline.** OAuth not required.
...
- **Ingestion correctness:** integration test ingests `2026_season_data/2026-04-25/2026-04-25-DB.axdb` and asserts:
  - 76 drivers, 622 runs in the app DB,
  - at least one `DNF` and one `RRN` row,
  - 7 distinct class codes.
...
|---|------------------------------------------------------------------------------------------------------|---------|-------|
| 1 | Who has MSR admin access on the RMR org? Needed to request OAuth creds + retrieve org ID.            | M2, M3  | TBD   |
