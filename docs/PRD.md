...
- Series scoring rules as data: `Season.plannedEvents` records the expected scoring-event count and `Season.minimumEvents` records the attendance required for an Official standing. Each Season points to a live `ScoringSystem` ruleset whose v3 policy owns `dropCount`, `dropTiming`, PAX-section behavior, and cone penalty. Qualification and dropped-score count are deliberately independent. Future regions or rule changes may still need configurable points formulas, tiebreakers, or single-car vs. multi-car rules.
