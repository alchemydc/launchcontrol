...
- Series scoring rules as data: RMR's qualifying-threshold formula (`floor(N/2) + 1` of N season events) and single-car-per-season constraint will live in code as of M1.13 (planned). Future regions or rule changes would want a per-season `RuleSet` (events-counted formula, points formula, drop-week count, tiebreakers, car-identity rule).
