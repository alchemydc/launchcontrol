...
- **Classing** — which car is eligible for which class, by model, generation, year range and trim. **League-specific**: PCA RMR's C1-C5/CS has nothing in common with SCCA Solo's SS/AS/BS. Distinct from **PAX**, which is the numeric index applied to a class once a car is already in it. The model is versioned repo data (`apps/web/src/data/classing/<league-slug>.json`), not DB rows — see the Classing feature section below and decision #19 in [BUILD.md](./BUILD.md)'s Decisions Log.
...
- **Driver / Competitor** — wants a clean mobile-responsive results dashboard (raw / PAX / class), and a way to view or share event media. May appear in more than one league; driver stats can be filtered per-league or aggregated across all of them, and are reachable directly from their own name in the header.
...
§1.3.1–1.3.2 and §1.3.4 are the original MVP scope; §1.3.3 covers the multi-league and ruleset-administration capabilities added in PR #99; §1.3.5 covers the per-league classing guide.
...
#### 1.3.5 Vehicle classing guide

Answers the question a prospective entrant asks before anything else: *what class does my car run in?* Previously answered only by a static HTML table generated out-of-band and published on the club's own site.

- **Per-league, per-season model** — classing rules are league-specific, so each league supplies its own model, and each class assignment names the seasons it applies to (a car can be reclassed between seasons — the Taycan moved from C5 to CS for 2025). A league with no model simply has no classing page and no classing tab; nothing else changes for it.
- **Public, deliberately** — `/l/[league]/classing` (legacy alias `/classing` for the default league) is the **one ungated league-scoped route**. A classing table contains no PII and no results; it is reference material a non-member reads while deciding whether to show up, and gating it would defeat its purpose. Every results route keeps its existing gate untouched.
- **Two surfaces** — a "find my class" picker (model → year → trim, resolving to a class) above the full class-by-class reference table for the selected season; and hover/tap cards on the class badges in the event, combined-event, and driver-history tables, listing that class's cars for that event's season with a link into the full guide.
- **Honest about what it can't decide** — the picker shows *every* matching class rather than silently choosing one when generation year ranges overlap, and surfaces engine-displacement conditions (a G-Series 911 Turbo is C1 only up to 3.3L) that a dropdown cannot resolve. Organizers have the final say, and the page says so.
- **Rulebook as versioned data** — the model is checked-in JSON regenerated from the upstream YAML via `pnpm --filter web classing:import --league <slug> <classing.yml>`, so a rules change is a reviewable diff. It is deliberately *not* DB data: unlike league/season/ruleset config it never varies between deployments, and it is read on every page that draws a class badge. See decision #19 in [BUILD.md](./BUILD.md)'s Decisions Log.
...
- **PII rule:** the full last name of any driver is used only transiently to compute the identity hash (at ingest) and the full-name hash (at ingest, and at MSR login to match a signed-in user to their own `Driver` row), and must never be persisted to the app DB, the session cookie, or any leaderboard rendering. A one-way digest of a name is not the name; storing the digest is in scope, storing the surname is not.
...
- **Classing correctness and reach:** the classing model is validated on load — a malformed checked-in file fails at boot with the offending path named, never as blank cells at request time. Per-season grouping is verified against the club's own published table (2026 output is line-for-line identical, including generation-half collapse and open-ended year ranges). The classing routes take **no** access gate, and a league without a classing model must 404 its classing route, hide its subnav tab, and render class badges exactly as before — with no hover card — rather than degrading them. Covered by `apps/web/tests/classing.test.ts`.
...
- An **SCCA Solo classing model** for RMsolo, so the classing guide and class hover cards light up for the second league. The registry takes one JSON file plus one line; the work is authoring and maintaining the model itself, which is far larger than PCA's (SS/AS/BS/CAM/Street Touring/Prepared/Modified, with far more vehicles).
- **Admin-editable classing**, if a league ever needs to correct a classing table without a deploy. Deliberately deferred — see decision #19 in [BUILD.md](./BUILD.md).
