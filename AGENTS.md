...
2. **`Driver.nameOnlyHash`** — matched only when **exactly one** `Driver` carries the hash **and that row is unclaimed**, the same "exactly one candidate or don't guess" rule ingest uses for merge/adopt. Both queries count over *every* row carrying the hash, never just the unclaimed ones: ambiguity is a property of the data, not of who claimed what, and filtering claimed rows out would hide a same-name row behind its owner and turn a genuine ≥2 into a false "exactly one" — returning another user's results to this viewer.
...
The `/me` card resolves the driver's leagues through `checkLeagueAccess` and summarizes only the ones this session may actually open — `/me` carries no league gate of its own, so aggregating blindly would show a count whose stats route would redirect the viewer.
