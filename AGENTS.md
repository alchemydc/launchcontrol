...
- **Second ingest source:** `src/lib/rmsolo-{index,parse,ingest}.ts` + `scripts/ingest-rmsolo.ts` scrape a club's RMsolo results page and parse its Full-results PDFs (`pdftotext -layout`) into the same Prisma models as the AxWare pipeline. Club-specific behavior — branding, `ACCESS_GATE`, `NAME_DISPLAY` — is env-driven via `src/lib/club-config.ts`, not hardcoded per deployment. `Driver.lastName` is only ever written by public-results sources (RMsolo); the AxWare pipeline never populates it.
